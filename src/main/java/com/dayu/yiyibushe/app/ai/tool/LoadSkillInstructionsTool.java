...
public class LoadSkillInstructionsTool implements AgentTool {
        ...
        return Map.of(
                ...
                "properties", Map.of(
                        "skillName", Map.of("type", "string",
                                "description", "技能名（技能目录中列出的 name），如 current-weather")),
    ...
    public Mono<ToolResultBlock> callAsync(ToolCallParam param) {
        ...
        if (StringUtilExt.isBlank(skillName)) {
            return Mono.just(ToolResultBlock.error("load-skill-instructions 缺少必填参数 skillName"));
        }

        SkillDocument skillDocument = skillDocumentLoader.getSkill(skillName.trim());
        ...
        if (Objects.nonNull(trace)) {
            ...
            trace.step(TracePhase.SKILL,
                    ...
                    skillDocument.name(), skillDocument.instructions().length());
